class Room < ActiveRecord::Base
  ...
  scope :time_range, lambda { |start_time, end_time| where("updated_at >= ? and updated_at <= ?", start_time, end_time) }
  ...
  def self.room(sessionId)
    opentok_config
    @fbConfig = \
              FB_CONFIG
              
    token = token(@opentok, sessionId)
  ...
  def self.sutalkLink( sessionId )
    ...
    "http://apps.facebook.com/" + \
                        "#{FB_CONFIG["canvas"]}/?sid=#{sessionId}"
  end
  ...
  def self.createOpenTokSession(create_new = true)

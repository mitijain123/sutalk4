module ApplicationHelper
  ...
  def sutalkLink(element_id,  session_id = '')
      room_link = (@room && @room[:sutalkLink]) || Room.sutalkLink(session_id)
       text_field_tag "#{element_id}", room_link,
  ...
  def newRoom?
    @params && @params[:sid].blank? && @params[:request_ids].blank?
  end
end
